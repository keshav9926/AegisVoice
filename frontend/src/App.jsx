        ...
        const defaultQuestions = [
          {
            ...
            "idealAnswer": "A strong answer should highlight: 1. React Context is a dependency injection tool, not a full state management solution. It passes data down the component tree to avoid prop-drilling. 2. Redux is a full state management system featuring a centralized store, actions, reducers, and middleware. 3. Performance: Context API triggers a re-render of all consumer components when its value changes, which can lead to performance bottlenecks for highly dynamic or frequent state updates. Redux allows components to select specific slices of state and only re-render when those specific slices change. 4. Choice: Choose Context for static or low-frequency updates (themes, user auth info). Choose Redux/Zustand for complex, high-frequency, or globally distributed application state."
          },
          {
            "id": "q2",
            ...
            "idealAnswer": "A strong answer should highlight: 1. REST uses fixed endpoints where the server determines response shape. GraphQL uses a single endpoint where client queries specify exact fields. 2. Over-fetching and Under-fetching are REST weaknesses solved by GraphQL's client-defined schemas. 3. Schema: GraphQL enforces a strongly typed schema contract. 4. Caching: REST caching is built into HTTP. GraphQL caching is complex because it uses POST request payloads, requiring client-side normalization libraries (like Apollo)."
          },
          {
            "id": "q3",
            ...
            "idealAnswer": "A strong answer should highlight: 1. Under the hood: Indexes are data structures (typically B-Trees) containing keys and pointers to rows, permitting O(log N) lookups instead of O(N) full table scans. 2. Read Performance: Indexing drastically speeds queries with filtering (WHERE), sorting (ORDER BY), or joins. 3. Write Performance Trade-off: Every write (INSERT, UPDATE, DELETE) requires updating the indexes, degrading write latency. 4. Storage Trade-off: Indexes consume disk space and memory buffer pools."
          }
        ];
